#!/bin/bash
# ==============================================================
#  AnyTLS 安装 / 卸载脚本（Ubuntu / Debian Systemd，分步版）
#  仓库：anytls/anytls-go
#
#  重要说明：
...
step_install_1() {
    ...
    if [ -f /etc/debian_version ]; then
        info "检测到 Ubuntu / Debian 系统，正在更新软件包列表..."
    else
        warn "未检测到 Ubuntu / Debian 系统，继续尝试安装依赖..."
    fi
    apt-get update -qq
    apt-get install -y -qq curl unzip openssl ca-certificates bash >/dev/null
    success "依赖安装完成（curl unzip openssl ca-certificates bash）"
}
...
step_install_4() {
    echo -e "\n${BOLD}[安装 4/6] 创建 Systemd 服务${RESET}"
...
RestartSec=3s
LimitNOFILE=65535
StandardOutput=append:/var/log/anytls-stdout.log
StandardError=append:/var/log/anytls-error.log
...
SERVICEEOF
    ...
    success "Systemd 服务文件写入完成并已重载配置"
...
step_install_5() {
    ...
    [ -f "$SERVICE_FILE" ] || error "服务文件不存在，请先执行安装步骤 4"
    ...
    success "AnyTLS 已设为开机自启并尝试启动"

    info "等待 2 秒后检查进程..."
    sleep 2

    info "当前服务状态："
    systemctl status anytls --no-pager || true
    ...
    if ss -tlnp 2>/dev/null | grep -q ":${PORT}"; then
        ...
        echo
        journalctl -u anytls -n 20 --no-pager 2>/dev/null || tail -n 20 /var/log/anytls-error.log 2>/dev/null || warn "日志文件不存在"
        echo
...
step_install_6() {
    ...
    echo -e "${BOLD}${GREEN}============================================${RESET}"
    echo
    echo -e "查看日志：${CYAN}journalctl -u anytls -f${RESET}  或  ${CYAN}tail -f /var/log/anytls-error.log${RESET}"
    echo -e "重启服务：${CYAN}systemctl restart anytls${RESET}"
    echo -e "停止服务：${CYAN}systemctl stop anytls${RESET}"
    echo -e "查看状态：${CYAN}systemctl status anytls${RESET}"
    echo
}
...
step_remove_1() {
    ...
    if [ -f "$SERVICE_FILE" ]; then
        ...
        warn "未找到 Systemd 服务文件，跳过"
    fi
}

step_remove_2() {
    echo -e "\n${BOLD}[卸载 2/5] 删除 Systemd 服务文件${RESET}"
...
show_main_menu() {
    ...
    echo -e "${BOLD}   Ubuntu / Debian 版本               ${RESET}"
    echo -e "${BOLD}======================================${RESET}"
    echo
...
show_install_menu() {
    ...
    echo -e "  ${CYAN}4${RESET}  创建 Systemd 服务"
    ...
    echo -e "  ${YELLOW}PORT=38443 bash $0 install 3${RESET}"
    echo
}
...
show_remove_menu() {
    ...
    echo -e "  ${CYAN}2${RESET}  删除 Systemd 服务文件"
