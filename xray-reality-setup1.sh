...
PRIVATE_KEY=$(echo "$PRIVATE_KEY" | grep -i "private" | awk '{print $NF}')
PUBLIC_KEY=$(echo  "$PUBLIC_KEY"  | grep -i "public"  | awk '{print $NF}')

# 兼容部分环境下变量捕获失败的 fallback 逻辑
if [ -z "$PRIVATE_KEY" ] || [ -z "$PUBLIC_KEY" ]; then
    PRIVATE_KEY=$(echo "$KEYPAIR" | grep -i "private" | cut -d' ' -f3)
    PUBLIC_KEY=$(echo "$KEYPAIR" | grep -i "public" | cut -d' ' -f3)
fi
...
# 启动服务
rc-service xray start  > /dev/null 2>&1 || error "Xray 启动失败"
